...
data_source = '20';
files_fail = dir(fullfile('../../../data/failure_data/', ['ce_' data_source '_*.txt']));
...
files_suc = dir(fullfile('../../../data/ce/', ['ce_' data_source '_test*.txt']));
...
for i = 1:n
    ...
    if i <= files_fail_size
        ...
        L_drop = [L_drop; 0];
        M_drop = [M_drop; M(end-5,[state_inx action_inx])];
        L_drop = [L_drop; 0];
    end
    if i == files_fail_size+1
        idx = floor(linspace(50, size(M,1)-50, 25)); %[200 300 550 570 750 1250 1500 1750 1800 1900];
...
clear data
data = [M_drop; M_normal];
labels = [L_drop; L_normal];
