...
void log2file(index_t n_train, index_t n_test, SGMatrix<float64_t> X, SGMatrix<float64_t> Y, SGMatrix<float64_t> X_test, SGMatrix<float64_t> mu, SGMatrix<float64_t> s2) {
	...
	for (index_t  i=0, j=0; i<n_test; i++, j+=2) {
        if (i < n_train)
            myfile << X[j] << " " << X[j+1] << " "  << Y[i] << " " << X_test[j] << " " << X_test[j+1] << " " << mu[i] << " " << s2[i] << endl;
        else
            myfile << 0 << " " << 0 << " "  << 0 << " " << X_test[j] << " " << X_test[j+1] << " " << mu[i] << " " << s2[i] << endl;
    }
...
void test()
{
	...
	for (index_t  i=0, j=0; i<n_train; i++, j+=2)
	{
		X[j]=CMath::random(-10, 10);
        X[j+1]=CMath::random(-10, 10);
        // X[j]=(double)rand()/RAND_MAX*20 - 10;
        // X[j+1]=(double)rand()/RAND_MAX*20 - 10;
		Y[i] = X[j]*X[j]*0.5 + X[j+1]*X[j+1]*0.25 + distribution(generator);
	}
    // X.display_matrix("X");
    // exit(0);

	for (index_t  i=0, j=0; i<n_test; i++, j+=2) {
        X_test[j]=CMath::random(-12, 12);
        X_test[j+1]=CMath::random(-12, 12);
    }
